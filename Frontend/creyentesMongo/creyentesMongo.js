...
async function loadData() {
    ...
    informacion.forEach((info) => {
        ...
        <div class="cookie-card">
            <span class="title">🏙️ ${info.nombre}</span>
            <p class="description">Edad : ${info.edad}</p>
            <p class="description">Nivel Formacion : ${info.nivelFormacion}</p>
            <p class="description">Nivel Ruta : ${info.nivelRuta}</p>
            <p class="description">Ministerio  : ${info.Ministerio}</p>
...
function borrar(e){
    if (e.target.classList.contains("eliminar")) {
        ...
        const idCreyente = e.target.getAttribute("id");
        const confir = confirm("Desea eliminar este Creyente?");
        if (confir) {
            deleteData(idCreyente);
        }
    }
}
...
async function getInfo(e){
    if (e.target.classList.contains("update")) {
        ...
        const {_id,nombre, edad, nivelFormacion, nivelRuta, Ministerio} = informacion;
    
        const nombreEdit = document.querySelector('#nombreEdit');
        const edadEdit = document.querySelector('#edadEdit');
        const nivelFormacionEdit = document.querySelector('#nivelFormacionEdit');
        const nivelRutaEdit = document.querySelector('#nivelRutaEdit');
        const ministerioEdit = document.querySelector('#ministerioEdit');
        const idEdit = document.querySelector('#idEdit');

        nombreEdit.value = nombre;
        edadEdit.value = edad
        nivelFormacionEdit.value = nivelFormacion,
        nivelRutaEdit.value = nivelRuta,
        ministerioEdit.value = Ministerio,
        idEdit.value = _id;
    }
};
...
function actualizar(e){
    ...
    const nivelFormacion = document.querySelector('#nivelFormacionEdit').value;
    const nivelRuta = document.querySelector('#nivelRutaEdit').value;
    const Ministerio = document.querySelector('#ministerioEdit').value;
