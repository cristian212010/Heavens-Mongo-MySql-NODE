    ...
    informacion.forEach((info) => {
        ...
        <div class="cookie-card">
            ...
            <div class="btnEditDelete">
                ...
                <button class="delete-button eliminar" id="${info.idBarrio}">
...
function insertInfo(e) {
  e.preventDefault();
  const nombreBarrio = document.querySelector("#nombre").value;
...
function borrar(e){
    if (e.target.classList.contains("eliminar")) {
        ...
        const idBarrio = e.target.getAttribute("id");
        const confir = confirm("Desea eliminar este Barrio?");
        if (confir) {
            deleteData(idBarrio);
        }
    }
...
async function getInfo(e){
    if (e.target.classList.contains("update")) {
        ...
        informacion.forEach(info => {
            const {idBarrio,nombreBarrio, idComuna} = info;
    
            const nombreEdit = document.querySelector('#nombreEdit');
            const idComunaEdit = document.querySelector('#idComunaEdit');
            const idEdit = document.querySelector('#idEdit');
    
            nombreEdit.value = nombreBarrio;
            idComunaEdit.value = idComuna
            idEdit.value = idBarrio;
        });
    }
};
...
function actualizar(e){
    ...
    const nombreBarrio = document.querySelector('#nombreEdit').value;
    const idComuna = document.querySelector('#idComunaEdit').value;
