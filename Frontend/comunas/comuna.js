...
async function loadData() {
    ...
    informacion.forEach((info) => {
        ...
        <div class="cookie-card">
            <span class="title">🏚️ ${info.nombreComuna}</span>
            <p class="description">Id Comuna: ${info.idComuna}</p>
            <p class="description">Nombre Municipio: ${info.nombreMunicipio}</p>
            <div class="btnEditDelete">
                <button class="edit-button update" id="${info.idComuna}" data-bs-toggle="modal" data-bs-target="#modalUpdate">
                ...
                <button class="delete-button eliminar" id="${info.idComuna}">
...
function insertInfo(e) {
  e.preventDefault();
  const nombreComuna = document.querySelector("#nombre").value;
...
function borrar(e){
    if (e.target.classList.contains("eliminar")) {
        ...
        const idComuna = e.target.getAttribute("id");
        const confir = confirm("Desea eliminar esta Comuna?");
        if (confir) {
            deleteData(idComuna);
        }
    }
...
async function getInfo(e){
    if (e.target.classList.contains("update")) {
        ...
        informacion.forEach(info => {
            const {idComuna,nombreComuna, idMunicipio} = info;
    
            const nombreEdit = document.querySelector('#nombreEdit');
            const idMunicipioEdit = document.querySelector('#idMunicipioEdit');
            const idEdit = document.querySelector('#idEdit');
    
            nombreEdit.value = nombreComuna;
            idMunicipioEdit.value = idMunicipio
            idEdit.value = idComuna;
        });
    }
};
...
function actualizar(e){
    ...
    const nombreComuna = document.querySelector('#nombreEdit').value;
    const idMunicipio = document.querySelector('#idMunicipioEdit').value;
