...
async function loadData() {
    ...
    informacion.forEach((info) => {
        ...
        <div class="cookie-card">
            <span class="title">🌎 ${info.nombreDepartamento}</span>
            <p class="description">Id Departamento: ${info.idDepartamento}</p>
            <div class="btnEditDelete">
                <button class="edit-button update" id="${info.idDepartamento}" data-bs-toggle="modal" data-bs-target="#modalUpdate">
                ...
                <button class="delete-button eliminar" id="${info.idDepartamento}">
...
function insertInfo(e) {
  e.preventDefault();
  const nombreDepartamento = document.querySelector("#nombre").value;
...
function borrar(e){
    if (e.target.classList.contains("eliminar")) {
        ...
        const confir = confirm("Desea eliminar este Departamento?");
        if (confir) {
            deleteData(idDepartamento);
        }
    }
}
...
async function getInfo(e){
    if (e.target.classList.contains("update")) {
        ...
        informacion.forEach(info => {
            const {idDepartamento,nombreDepartamento} = info;
    
            const nombreEdit = document.querySelector('#nombreEdit');
            const idEdit = document.querySelector('#idEdit');
    
            nombreEdit.value = nombreDepartamento;
            idEdit.value = idDepartamento;
        });
    }
};
...
function actualizar(e){
    ...
    const nombreDepartamento = document.querySelector('#nombreEdit').value;
