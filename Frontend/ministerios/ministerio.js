...
async function loadData() {
    ...
    informacion.forEach((info) => {
        ...
        <div class="cookie-card">
            <span class="title">🏙️ ${info.Ministerio}</span>
...
function insertInfo(e) {
  e.preventDefault();
  const Ministerio = document.querySelector("#nombre").value;
...
function borrar(e){
    if (e.target.classList.contains("eliminar")) {
        ...
        const idMinisterio = e.target.getAttribute("id");
        const confir = confirm("Desea eliminar este Ministerio?");
        if (confir) {
            deleteData(idMinisterio);
        }
    }
}
...
async function getInfo(e){
    if (e.target.classList.contains("update")) {
        ...
        const {_id,Ministerio} = informacion;
    
        const nombreEdit = document.querySelector('#nombreEdit');
        const idEdit = document.querySelector('#idEdit');

        nombreEdit.value = Ministerio;
        idEdit.value = _id;
    }
};
...
function actualizar(e){
    ...
    const Ministerio = document.querySelector('#nombreEdit').value;
