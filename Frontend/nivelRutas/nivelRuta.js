...
async function loadData() {
    ...
    informacion.forEach((info) => {
        ...
        <div class="cookie-card">
            <span class="title">🏙️ ${info.nivelRuta}</span>
...
function insertInfo(e) {
  e.preventDefault();
  const nivelRuta = document.querySelector("#nombre").value;
...
function borrar(e){
    if (e.target.classList.contains("eliminar")) {
        ...
        const idNivelRuta = e.target.getAttribute("id");
        const confir = confirm("Desea eliminar este nivel de ruta?");
        if (confir) {
            deleteData(idNivelRuta);
        }
    }
}
...
async function getInfo(e){
    if (e.target.classList.contains("update")) {
        ...
        const {_id,nivelRuta} = informacion;
    
        const nivelRutaEdit = document.querySelector('#nombreEdit');
...
function actualizar(e){
    ...
    const nivelRuta = document.querySelector('#nombreEdit').value;
